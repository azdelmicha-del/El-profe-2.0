...
function buildSpecialistInstructions(exactFormat, user = null) {
    ...
    if (exactFormat && exactFormat.variables && exactFormat.variables.length > 0) {
        ...
        const userInfo = user ? `
=== DATOS DEL PERFIL Y CONTEXTO TEMPORAL ===
HOY ES: ${currentDateStr} (Usa esta fecha para calcular fechas como "mañana", "el lunes", etc.)
Nombre: ${user.name || '-'}
Cédula: ${user.cedula || '-'}
                ...
                if (orquestadorRes.ok) {
                    ...
                    if (responseMessage.tool_calls) {
                        ...
                        if (finalJsonFromSpecialist) {
                            messages.push({
                                role: 'system',
                                content: 'El especialista ha generado la planificación con éxito. ESTÁ ESTRICTAMENTE PROHIBIDO mostrar la planificación, tablas o contenido del especialista en tu mensaje de WhatsApp. Simplemente dile al profesor "✅ Tu documento está listo" y DEBES incluir [GENERATE_DOCX] al final de tu mensaje para que el servidor genere y entregue el archivo Word.'
                            });
                        }
