...
async function run() {
    ...
    if (knowledgeItems && knowledgeItems.length > 0) {
        ...
        globalKnowledgeBlock += '\nUSA ESTA BASE COMO REFERENCIA.\n';
    }

    const formats = await db.collection('doc_formats').find({}).toArray();
    let dynamicInstructions = '\n\n### ESTRUCTURAS JSON REQUERIDAS POR PLANTILLA\nDependiendo de la plantilla que uses, DEBES generar tu respuesta OBLIGATORIAMENTE usando el formato JSON exacto correspondiente:\n';
